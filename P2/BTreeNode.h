...
class BTLeafNode {
  public:
    ...
    * the final 4 bytes are used to store the count of records in the file.
    * each pair of record stores a RecordId, which is 8 bytes(two integes), and a key which is 4 bytes.
    * so each record is 12 bytes long.
    * To make the coding convenient and easy to read, set constant of the MAX_NUM_LEAF and LEAF_ENTRY_SIZE.
    * both number is calculated ahead of time.
    */

    static const int MAX_NUM_LEAF = 84;
...
class BTNonLeafNode {
  public:
    ...
    * Each Pagefile is 1024 bytes
    * the final 4 bytes are used to store the count of records in the file.
    * each pair of record stores a PageId, which is 4 bytes(Integer), and a key which is 4 bytes.
    * so each record is 8 bytes long.
    * To make the coding convenient and easy to read, set constant of the MAX_NUM_NONLEAF and NONLEAF_ENTRY_SIZE.
    * both number is calculated ahead of time.
    */

    static const int MAX_NUM_NONLEAF = 99; //odd is better for split;
    static const int NONLEAF_ENTRY_SIZE = 8;
  /**
    * constructor that initializes the node
    */
    BTNonLeafNode();
